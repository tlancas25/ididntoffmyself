...
- `surface`: one of the 15 in `agent-roster.md`
...
Because you mark `exploitable_now: false`, `tools/synthesize.py` routes recon findings into the **Hardening Recommendations** section. That's fine — recon's purpose is the map, and the map belongs alongside posture findings, not in the fix-now list.
