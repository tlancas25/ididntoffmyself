...
## Role
Last agent to run. Drive `tools/synthesize.py`, then add the human-judgment layer that the tool can't mechanize: dedup resolution, Fix-These-First curation, and specialist-prompt feedback for the next trial.

## Primary action
Run:

```bash
python tools/synthesize.py targets/<slug>-<ts>/
```
...
- Writes `report.md`.
- Appends `(auto-drafted)` entries to `promotion-queue/README.md` for `novel_pattern: true` findings, with dedup by finding-id and surface+title combo.
...
- Tooling gaps (synthesize.py dedup false-positives, chain-inference gaps).
